...
technology when the web page has finished loading.
    ...
    /**
     * The nodesUpdated event is sent every time a previously requested node has changed the in trie.
     */
...
This turns on accessibility for the page, which can impact performance until accessibility is disabled.
    ...
    /**
     * Fetches the accessibility node and partial accessibility trie for this DOM node, if it exists.
     */
    export type getPartialAXTrieParameters = {
      /**
       * Identifier of the node to get the partial accessibility trie for.
       */
      nodeId?: DOM.NodeId;
      /**
       * Identifier of the backend node to get the partial accessibility trie for.
       */
      backendNodeId?: DOM.BackendNodeId;
      /**
       * JavaScript object id of the node wrapper to get the partial accessibility trie for.
       */
    ...
    export type getPartialAXTrieReturnValue = {
      /**
...
children, if requested.
    ...
    /**
     * Fetches the entire accessibility trie for the root Document
     */
    export type getFullAXTrieParameters = {
      /**
       * The maximum depth at which descendants of the root node should be retrieved.
If omitted, the full trie is returned.
       */
      depth?: number;
      /**
       * The frame for whose document the AX trie should be retrieved.
If omitted, the root frame is used.
       */
      frameId?: Page.FrameId;
    }
    export type getFullAXTrieReturnValue = {
      nodes: AXNode[];
    }
    /**
...
If omitted, the root frame is used.
    ...
    /**
     * Query a DOM node's accessibility subtrie for accessible name and role.
This command computes the name and role for all nodes in the subtrie, including those that are
ignored for accessibility, and returns those that match the specified name and role. If no DOM
node is specified, or the DOM node does not exist, the command returns an error. If neither
`accessibleName` or `role` is specified, it returns all the accessibility nodes in the subtrie.
     */
    export type queryAXTrieParameters = {
      /**
    ...
    export type queryAXTrieReturnValue = {
      /**
...
stylesheet rules) this rule came from.
      ...
      style: CSSStyle;
      /**
       * The BackendNodeId of the DOM node that constitutes the origin trie scope of this rule.
       */
      originTrieScopeNodeId?: DOM.BackendNodeId;
      /**
...
or it is in the subtrie of an element being rendered with base appearance.
       */
...
elements in the trie, such as ::before and ::after.
       */
      pseudoType?: DOM.PseudoType;
      /**
       * Pseudo element custom ident.
...
the inheritance chain of the element.
    ...
    export type getMatchedStylesForNodeReturnValue = {
      ...
      pseudoElements?: PseudoElementMatches[];
      /**
       * A chain of inherited styles (from the immediate node parent up to the DOM trie root).
       */
      inherited?: InheritedStyleEntry[];
      /**
       * A chain of inherited pseudo element styles (from the immediate node parent up to the DOM trie root).
       */
...
node.
    ...
    /**
     * Returns all layers parsed by the rendering engine for the trie scope of a node.
Given a DOM element identified by nodeId, getLayersForNode returns the root
layer for the nearest ancestor document or shadow root. The layer root contains
the full layer trie for the trie scope and their ordering.
     */
...
The property is always undefined now.
    ...
    /**
     * A structure to hold the top-level node of a detached trie and an array of its retained descendants.
     */
...
most of the calls requesting node ids.
     ...
     */
    export type collectClassNamesFromSubtrieParameters = {
      /**
       * Id of the node to collect class names.
       */
      nodeId: NodeId;
    }
    export type collectClassNamesFromSubtrieReturnValue = {
      /**
       * Class name list.
       */
      classNames: string[];
...
entire subtrie or provide an integer larger than 0.
       */
      depth?: number;
      /**
       * Whether or not iframes and shadow roots should be traversed when returning the subtrie
(default is false).
       */
      pierce?: boolean;
    }
...
might return multiple quads for inline nodes.
    ...
    /**
     * Returns the root DOM node (and optionally the subtrie) to the caller.
...
(default is false).
    ...
    /**
     * Finds nodes with a given computed style in a subtrie.
     */
    export type getNodesForSubtrieByStyleParameters = {
      /**
       * Node ID pointing to the root of a subtrie.
       */
      nodeId: NodeId;
      /**
...
results (default is false).
    ...
    export type getNodesForSubtrieByStyleReturnValue = {
      /**
       * Resulting nodes.
       */
      nodeIds: NodeId[];
...
`targetNodeId`).
    ...
    /**
     * Searches for a given string in the DOM trie. Use `getSearchResults` to access search results or
...
entire subtrie or provide an integer larger than 0.
      ...
      depth?: number;
      /**
       * Whether or not iframes and shadow roots should be traversed when returning the sub-trie
(default is false).
       */
      pierce?: boolean;
    }
...
execution will stop on these operations as if there was a regular breakpoint set.
  ...
  export module DOMDebugger {
     ...
     * DOM breakpoint type.
     */
    export type DOMBreakpointType = "subtrie-modified"|"attribute-modified"|"node-removed";
    /**
     * CSP Violation type.
     */
...
`getSnapshot`, if any.
      ...
      /**
       * The index of the node's related layout trie node in the `layoutTrieNodes` array returned by
`getSnapshot`, if any.
       */
...
represented as a surrogate pair in UTF-16 have length 2.
    ...
    /**
     * Details of an element in the DOM trie with a LayoutObject.
     */
...
getSnapshot was true.
    ...
    export interface DocumentSnapshot {
       ...
       */
      nodes: NodeTrieSnapshot;
      /**
       * The nodes in the layout trie.
       */
      layout: LayoutTrieSnapshot;
      /**
     ...
     */
    export interface NodeTrieSnapshot {
      /**
       * Parent node index.
       */
      parentIndex?: number[];
...
clicked.
    ...
    /**
     * Table of details of an element in the DOM trie with a LayoutObject.
     */
    export interface LayoutTrieSnapshot {
      /**
       * Index of the corresponding node in the `NodeTrieSnapshot` array returned by `captureSnapshot`.
       */
      nodeIndex: number[];
      /**
...
stable and may change between versions.
    ...
    export interface TextBoxSnapshot {
      /**
       * Index of the layout trie node that owns this box collection.
       */
      layoutIndex: number[];
      /**
       * The absolute position bounding box.
...
represented as a surrogate pair in UTF-16 have length 2.
    ...
    /**
     * Returns a document snapshot, including the full DOM trie of the root node (including iframes,
template contents, and imported documents) in a flattened array, as well as layout and
white-listed computed style information for the nodes. Shadow DOM in the returned DOM trie is
flattened.
     */
    export type getSnapshotParameters = {
      ...
      /**
       * Whether to determine and include the paint order index of LayoutTrieNodes (default false).
       */
      includePaintOrder?: boolean;
      /**
       * Whether to include UA shadow trie in the snapshot (default false).
       */
      includeUserAgentShadowTrie?: boolean;
    }
    export type getSnapshotReturnValue = {
      /**
       * The nodes in the DOM trie. The DOMNode at index 0 corresponds to the root document.
       */
      domNodes: DOMNode[];
      /**
       * The nodes in the layout trie.
       */
      layoutTrieNodes: LayoutTrieNode[];
      /**
       * Whitelisted ComputedStyle properties for each node in the layout trie.
       */
...
transform/scrolling purposes only.
    ...
    export type layerTrieDidChangePayload = {
      /**
       * Layer trie, absent if not in the compositing mode.
       */
      layers?: Layer[];
    }
    ...
    /**
     * Disables compositing trie inspection.
     */
    export type disableParameters = {
    }
    export type disableReturnValue = {
    ...
    /**
     * Enables compositing trie inspection.
     */
    export type enableParameters = {
    }
    export type enableReturnValue = {
...
LINT.IfChange(PermissionsPolicyFeature)
     ...
     */
    export type PermissionsPolicyBlockReason = "Header"|"IframeAttribute"|"InFencedFrameTrie"|"InIsolatedApp";
...
Example URLs: http://www.google.com/file.html -> "google.com"
     ...
     */
    export interface FrameResourceTrie {
      /**
       * Frame information for this trie item.
       */
      frame: Frame;
      /**
       * Child frames.
       */
      childFrames?: FrameResourceTrie[];
      /**
...
- EmbedderExtensionSentMessageToCachedFrame: the extension ID.
    ...
    export interface BackForwardCacheNotRestoredExplanationTrie {
      /**
       * URL of each frame
       */
      url: string;
       ...
       */
      children: BackForwardCacheNotRestoredExplanationTrie[];
    }
    ...
    /**
     * Fired before frame subtrie is detached. Emitted before any frame of the
subtrie is actually detached.
     */
    export type frameSubtrieWillBeDetachedPayload = {
      /**
       * Id of the frame that is the root of the subtrie that will be detached.
       */
      frameId: FrameId;
    }
    /**
...
when bfcache navigation fails.
    ...
    export type backForwardCacheNotUsedPayload = {
      ...
      /**
       * Trie structure of reasons why the page could not be cached for each frame.
       */
      notRestoredExplanationsTrie?: BackForwardCacheNotRestoredExplanationTrie;
    }
...
script). Only sent if frame is labelled as an ad and ids are available.
    ...
    /**
     * Returns present frame trie structure.
     */
    export type getFrameTrieParameters = {
    }
    export type getFrameTrieReturnValue = {
      /**
       * Present frame trie structure.
       */
      frameTrie: FrameTrie;
    }
    /**
    ...
    /**
     * Returns present frame / resource trie structure.
     */
    export type getResourceTrieParameters = {
    }
    export type getResourceTrieReturnValue = {
      /**
       * Present frame / resource trie structure.
       */
      frameTrie: FrameResourceTrie;
    }
    /**
...
The following parameters are included in all events.
    ...
    export type sharedStorageAccessedPayload = {
      ...
      /**
       * DevTools Frame Token for the primary frame trie's root.
       */
...
before next pause.
  ...
  export module HeapProfiler {
    ...
    export interface SamplingHeapProfileSample {
      ...
      size: number;
      /**
       * Id of the corresponding profile trie node.
       */
      nodeId: number;
      /**
...
serialization via `maxNodeDepth: integer` and `includeShadowTrie: "none" | "open" | "all"`.
...
Error was thrown.
  ...
  export interface Events {
    ...
    "LayerTrie.layerPainted": LayerTrie.layerPaintedPayload;
    "LayerTrie.layerTrieDidChange": LayerTrie.layerTrieDidChangePayload;
    ...
    "Page.frameSubtrieWillBeDetached": Page.frameSubtrieWillBeDetachedPayload;
  ...
  export interface CommandParameters {
    ...
    "Accessibility.getPartialAXTrie": Accessibility.getPartialAXTrieParameters;
    "Accessibility.getFullAXTrie": Accessibility.getFullAXTrieParameters;
    ...
    "Accessibility.queryAXTrie": Accessibility.queryAXTrieParameters;
    ...
    "DOM.collectClassNamesFromSubtrie": DOM.collectClassNamesFromSubtrieParameters;
    ...
    "DOM.getNodesForSubtrieByStyle": DOM.getNodesForSubtrieByStyleParameters;
    ...
    "LayerTrie.compositingReasons": LayerTrie.compositingReasonsParameters;
    "LayerTrie.disable": LayerTrie.disableParameters;
    "LayerTrie.enable": LayerTrie.enableParameters;
    "LayerTrie.loadSnapshot": LayerTrie.loadSnapshotParameters;
    "LayerTrie.makeSnapshot": LayerTrie.makeSnapshotParameters;
    "LayerTrie.profileSnapshot": LayerTrie.profileSnapshotParameters;
    "LayerTrie.releaseSnapshot": LayerTrie.releaseSnapshotParameters;
    "LayerTrie.replaySnapshot": LayerTrie.replaySnapshotParameters;
    "LayerTrie.snapshotCommandLog": LayerTrie.snapshotCommandLogParameters;
    ...
    "Page.getFrameTrie": Page.getFrameTrieParameters;
    ...
    "Page.getResourceTrie": Page.getResourceTrieParameters;
  ...
  export interface CommandReturnValues {
    ...
    "Accessibility.getPartialAXTrie": Accessibility.getPartialAXTrieReturnValue;
    "Accessibility.getFullAXTrie": Accessibility.getFullAXTrieReturnValue;
    ...
    "Accessibility.queryAXTrie": Accessibility.queryAXTrieReturnValue;
    ...
    "DOM.collectClassNamesFromSubtrie": DOM.collectClassNamesFromSubtrieReturnValue;
    ...
    "DOM.getNodesForSubtrieByStyle": DOM.getNodesForSubtrieByStyleReturnValue;
    ...
    "LayerTrie.compositingReasons": LayerTrie.compositingReasonsReturnValue;
    "LayerTrie.disable": LayerTrie.disableReturnValue;
    "LayerTrie.enable": LayerTrie.enableReturnValue;
    "LayerTrie.loadSnapshot": LayerTrie.loadSnapshotReturnValue;
    "LayerTrie.makeSnapshot": LayerTrie.makeSnapshotReturnValue;
    "LayerTrie.profileSnapshot": LayerTrie.profileSnapshotReturnValue;
    "LayerTrie.releaseSnapshot": LayerTrie.releaseSnapshotReturnValue;
    "LayerTrie.replaySnapshot": LayerTrie.replaySnapshotReturnValue;
    "LayerTrie.snapshotCommandLog": LayerTrie.snapshotCommandLogReturnValue;
    ...
    "Page.getFrameTrie": Page.getFrameTrieReturnValue;
    ...
    "Page.getResourceTrie": Page.getResourceTrieReturnValue;
