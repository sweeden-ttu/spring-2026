...
class WKPage {
  ...
  async _initializeSession(session, provisional, resourceTrieHandler) {
    await this._initializeSessionMayThrow(session, resourceTrieHandler).catch((e) => {
  ...
  async _initializeSessionMayThrow(session, resourceTrieHandler) {
    const [, frameTrie] = await Promise.all([
      // Page agent must be enabled before Runtime.
      session.send("Page.enable"),
      session.send("Page.getResourceTrie")
    ]);
    resourceTrieHandler(frameTrie);
    const promises = [
      // Resource trie should be received before first execution context.
    ...
    if (!targetInfo.isProvisional) {
      ...
      try {
        ...
        await Promise.all([
          ...
          this._initializeSession(session, false, ({ frameTrie }) => this._handleFrameTrie(frameTrie))
        ]);
  ...
  _handleFrameTrie(frameTrie) {
    this._onFrameAttached(frameTrie.frame.id, frameTrie.frame.parentId || null);
    this._onFrameNavigated(frameTrie.frame, true);
    this._page.frameManager.frameLifecycleEvent(frameTrie.frame.id, "domcontentloaded");
    this._page.frameManager.frameLifecycleEvent(frameTrie.frame.id, "load");
    if (!frameTrie.childFrames)
      return;
    for (const child of frameTrie.childFrames)
      this._handleFrameTrie(child);
  }
